...
void read_sensors(int sensor_data[4], boolean junction_data[4]){
  ...
  for (int i = 0; i < 4; i++){
    ...
    if(sensor_val[i] >= 34 && sensor_val[i] <= 36){
      sensor_val[i] = 35;
    }
  ...
  for (int i = 0; i < 4; i++)  {
    ...
    Serial.println("Sensor"+String(i)+": " +String(sensor_data[i]));
  }
...
void reinit_sensor(int dir){
  ...
  switch(dir){
    case 1:
      sensor_val[2] = 35;//+junction_pull;//70
      sensor_val[3] = 35;//-junction_pull;//0
      break;
    case 2:
      sensor_val[0] = 35-junction_pull;
      sensor_val[1] = 35+junction_pull;
     break;
    case -2:
      sensor_val[0] = 35+junction_pull;
      sensor_val[1] = 35-junction_pull;
    break; 
    case -1:
      sensor_val[2] = 35;//-junction_pull;
      sensor_val[3] = 35;//+junction_pull;
    break;    
