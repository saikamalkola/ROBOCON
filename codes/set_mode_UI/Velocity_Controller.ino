...
void cal_wheel_PID()
{
  for (int i = 0; i < 4; i++) {
    ...
    //    }
    if((wheel_I[i] + error[i]) > Imax)
    {
      wheel_I[i] = Imax;
    }
    else if((wheel_I[i]+ error[i]) < Imin)
    {
      wheel_I[i] = Imin;
    }
    else
    {
      wheel_I[i] = wheel_I[i] + error[i] ;
    }
    wheel_I[i] = wheel_I[i] + wheel_error[i];
    ...
    {
      w[i] = (w[i] * conv_factor[1]) - 70;
    }
    if (abs(w[i]) > 390 && abs(w[i]) <= 420)
    {
      w[i] = (w[i] * conv_factor[2]) - 239;
    }
    if (abs(w[i]) > 420 && abs(w[i]) < 480)
    {
      w[i] = (w[i] * conv_factor[3]) - 995;
    }
  }
}
